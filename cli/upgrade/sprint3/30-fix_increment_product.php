...
$fix_time = strtotime('2014-05-30 13:39:00');

$db = Database::factory();

$total_sql = "SELECT count(*) total FROM `product` WHERE id in(3221522,4814586,3270088,3359950,4814587,3270109,4814588,4814589)";
...
$per_page = 100;
while($start <= $total) {
	$query_sql = "SELECT * from `product` WHERE id in (3221522,4814586,3270088,3359950,4814587,3270109,4814588,4814589) limit $start,$per_page";
	...
	foreach ($products as $product) {
	    ...
	    //替换或新建product
	    $replace_sql = "REPLACE INTO `product_foobar` (vendor_id,unit_price,vendor_note,approver_id,approve_date,publisher_id,publish_date,ctime,mtime,name,manufacturer,catalog_no,model,spec,package,type,category_id,keywords,description,stock_status,id,expire_date,freeze_reasons,sale_volume,brand,supply_time,market_price,_extra) (SELECT vendor_id,unit_price,vendor_note,approver_id,approve_date,publisher_id,publish_date,ctime,mtime,name,manufacturer,catalog_no,model,spec,package,type,category_id,keywords,description,in_stock,id,expire_date,freeze_reasons,sale_volume,brand,lead_time,market_price,_extra FROM `product` WHERE id={$id})";
