...
foreach ($types as $type => $foo) {
	...
	while (true) {
		$products = $db->query("SELECT product.*,vendor.name as vendor_name,vendor.short_name as vendor_short_name FROM `product` LEFT JOIN vendor  ON (vendor.id=product.vendor_id) WHERE product.vendor_id = %d AND product.type='$type' limit %d, %d", $vid, $start, $per_page)->rows();
