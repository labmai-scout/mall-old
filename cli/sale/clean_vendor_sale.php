...
/*
 * file clean_vendor_sale.php
 * author Yu Li <yu.li@geneegroup.com>
 * date 2014-09-04
 *
 * brief 根据vendor_id 删除促销信息
 * usage SITE_ID=nankai php clean_vendor_sale.php 125
 */
...
//先改变数据的价格
$db->query("UPDATE `product` SET `unit_price`=`product`.`orig_price` WHERE `orig_price` > 0 AND `vendor_id`=$vid");
//刷新索引
update_sphinx_by_orig_price($vid);
...
function update_sphinx_by_orig_price($vid=0) {
    ...
    foreach ($types as $type => $foo) {
        ...
        while (true) {
            ...
            (vendor.id=product.vendor_id) WHERE product.type='$type' AND product.vendor_id=$vid AND sale_info != '' limit %d, %d", $start, $per_page)->rows();
